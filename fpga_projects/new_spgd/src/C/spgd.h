...
struct SPGD_t
{
	...
	double ADC_MAX_NEG_VAL;
	double ADC_VOLTAGE_STEP;
	double PERT_A;
	double PERT_B;
	uint32_t U_A_P_ndx;
	uint32_t U_B_P_ndx;
	...
	double * ADC_float_array;
	uint32_t * ADC_code_array;
	int * samps_array;	
};
...
struct SPGD_t * make_ADC_uint_arr(struct SPGD_t *);
struct SPGD_t * make_ADC_float_arr(struct SPGD_t *, double, double);
float ran1(long *);
float gasdev(long *);
