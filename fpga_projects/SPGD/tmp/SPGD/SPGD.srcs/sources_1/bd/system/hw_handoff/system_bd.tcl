...
# module references:
# ADC_DAC_LOOP, LED_CONTROLLER, axi_cfg_register, ADC_REG
...
proc create_hier_cell_PS7 { parentCell nameHier } {
  ...
  connect_bd_net -net ADC_DAC_LOOP_GP_OUT [get_bd_pins gpio2_io_i] [get_bd_pins axi_gpio_0/gpio2_io_i]
...
proc create_root_design { parentCell } {
  ...
  # Create instance: ADC_DAC_LOOP, and set properties
  set block_name ADC_DAC_LOOP
  set block_cell_name ADC_DAC_LOOP
  if { [catch {set ADC_DAC_LOOP [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $ADC_DAC_LOOP eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
  ...
  set block_cell_name LED_CONTROLLER
  if { [catch {set LED_CONTROLLER [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $LED_CONTROLLER eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
  ...
  # Create instance: axi_cfg_register, and set properties
  set block_name axi_cfg_register
  set block_cell_name axi_cfg_register
  if { [catch {set axi_cfg_register [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $axi_cfg_register eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
  ...
  connect_bd_intf_net -intf_net ps7_0_axi_periph_M01_AXI [get_bd_intf_pins PS7/M01_AXI] [get_bd_intf_pins axi_cfg_register/s_axi]

  # Create port connections
  connect_bd_net -net ADC_DAC_LOOP_DACA_CODE_OUT [get_bd_ports dac_dat_a_o] [get_bd_pins ADC_DAC_LOOP/DACA_CODE_OUT]
  connect_bd_net -net ADC_DAC_LOOP_DACB_CODE_OUT [get_bd_ports dac_dat_b_o] [get_bd_pins ADC_DAC_LOOP/DACB_CODE_OUT]
  connect_bd_net -net ADC_DAC_LOOP_DONE [get_bd_pins ADC_DAC_LOOP/DONE] [get_bd_pins LED_CONTROLLER/val]
  connect_bd_net -net ADC_DAC_LOOP_GP_OUT [get_bd_pins ADC_DAC_LOOP/GP_OUT] [get_bd_pins PS7/gpio2_io_i]
  connect_bd_net -net ADC_DAC_LOOP_val_0 [get_bd_ports dac_spi_clk_o] [get_bd_ports dac_spi_csb_o] [get_bd_ports dac_spi_reset_o] [get_bd_pins ADC_DAC_LOOP/val_0]
  connect_bd_net -net ADC_DAC_LOOP_val_1 [get_bd_ports dac_spi_sdio_o] [get_bd_pins ADC_DAC_LOOP/val_1]
  connect_bd_net -net ADC_REG_a_data_out [get_bd_pins ADC_BLOCK/a_data_out] [get_bd_pins ADC_DAC_LOOP/ADC_DATA_IN]
  connect_bd_net -net ADC_REG_adc_clk [get_bd_pins ADC_BLOCK/adc_clk] [get_bd_pins ADC_DAC_LOOP/ADC_CLK]
  connect_bd_net -net LED_CONTROLLER_led_o [get_bd_ports led_o] [get_bd_pins LED_CONTROLLER/led_o]
  ...
  connect_bd_net -net axi_cfg_register_cfg_data [get_bd_pins ADC_DAC_LOOP/CFG_IN] [get_bd_pins axi_cfg_register/cfg_data]
  connect_bd_net -net axi_gpio_0_gpio_io_o [get_bd_pins ADC_DAC_LOOP/GP_IN] [get_bd_pins PS7/gpio_io_o]
  connect_bd_net -net processing_system7_0_FCLK_CLK0 [get_bd_pins PS7/FCLK_CLK0] [get_bd_pins axi_cfg_register/aclk]
  connect_bd_net -net rst_ps7_0_50M_peripheral_aresetn [get_bd_pins PS7/S00_ARESETN] [get_bd_pins axi_cfg_register/aresetn]

  # Create address segments
  assign_bd_address -offset 0x43C00000 -range 0x00010000 -target_address_space [get_bd_addr_spaces PS7/processing_system7_0/Data] [get_bd_addr_segs axi_cfg_register/s_axi/reg0] -force
